...
public class NoticeDao {
	...
	// 직원번호를 전달 받아서 해당 직원에게 올 알람 갯수를 반환한다.
	public int getTotalNoticeRowsByEmpNo(int empNo) {
		return (Integer)SqlMapper.selectOne("mypages.getTotalNoticeRowsByEmpNo", empNo);
	}
	
	// 알람번호를 전달 받아서 해당 알람 게시글 정보를 반환받는다.
	@SuppressWarnings("unchecked")
	public List<MyNoticeDto> getNoticeByNo(int noticeNo){
		return (List<MyNoticeDto>)SqlMapper.selectList("mypages.getNoticeByNo", noticeNo);
	}
